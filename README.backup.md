...
[![Auto Update](https://img.shields.io/badge/auto--update-daily-brightgreen.svg)](https://github.com/thomas0829/Seattle-Source-Ranker/actions)

> **Note**: This project is in **Beta**. Previous versions are tagged as `Beta-v1.0` through `Beta-v3.0`.
...
- **GraphQL Search API** - Efficient user discovery with 5000 requests/hour per token
...
- **Automated Updates** - GitHub Actions workflow runs daily at midnight (Seattle time)
- **Scalable** - Successfully collected 447,533 projects from 23,371 Seattle developers
...
**Latest Collection Stats (Nov 16, 2025):**
- Total Projects: **447,533**
- Total Stars: **2,166,692**
- Users Searched: **27,462**
- Users with Valid Projects: **23,371** (85.1%)
- Success Rate: **99.996%** (1 user failed)
- Collection Method: **GraphQL Search API** (76 pre-optimized filters)
- Automation: **GitHub Actions** (daily at midnight Seattle time)
...
│   │   └── App.css                # Styling
│   └── package.json
├── data/                          # Collection output files (auto-cleaned)
│   ├── ranked_project_local_seattle.json   # Sorted by score
│   ├── ranked_by_language_seattle.json     # Grouped by language
│   ├── seattle_projects_*.json             # Raw collection (latest only)
│   └── seattle_users_*.json                # User list (latest only)
├── docs/                          # Documentation
...
The project features fully automated daily data collection and website deployment:

### Features
- **Scheduled Execution** - Runs daily at midnight (Seattle time)
- **Complete Automation** - Collection → Ranking → README Update → Website Deploy
- **Data Management** - Automatically cleans old files, keeps only latest
- **Failure Protection** - Only replaces data if collection succeeds
- **Seattle Timezone** - All timestamps use `America/Los_Angeles` timezone

### Workflow Steps
1. **User Search** - GraphQL API with 76 pre-optimized filters (~27K users)
2. **Data Collection** - Distributed processing with 8 workers (30K users, ~90 min)
3. **Generate Rankings** - Create sorted and language-grouped data
4. **Verify Success** - Check collection completed successfully
5. **Clean Old Data** - Remove previous `seattle_projects_*.json` and `seattle_users_*.json`
6. **Update README** - Auto-update statistics in this file
7. **Deploy Website** - Build React app and deploy to GitHub Pages
8. **Commit Data** - Push new data files to repository

### Setup
1. Add GitHub Secrets (Settings → Secrets → Actions):
   - `GITHUB_TOKEN_1` through `GITHUB_TOKEN_6` (your Personal Access Tokens)
2. Enable GitHub Pages (Settings → Pages → Deploy from `gh-pages` branch)
3. That's it! Workflow runs automatically every day

### Manual Trigger
Go to **Actions** → **Collect Data and Deploy Website** → **Run workflow**
