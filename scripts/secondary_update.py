...
def secondary_update(input_file=None, batch_size=100):
    ...
    print(f"📦 Split into {total_batches:,} batches ({batch_size} repos each)")
    print(f"🚀 Dispatching tasks to Celery workers...")
    print()
    ...
    if repos_to_remove:
        print()
        print(f"🗑️  Removing {len(repos_to_remove)} inaccessible repos...")
    ...
    print(f"Processing rate:          {total_projects/elapsed_time:.1f} repos/sec")
    print("=" * 70)
    print()
    ...
    print()
    print(f"🎉 Secondary update complete! {len(projects):,} verified repos remain.")
...
def main():
    ...
    try:
        ...
        if 'worker' in result.stdout:
            worker_count = result.stdout.count('@')
            print(f"✅ {worker_count} Celery workers detected")
        else:
