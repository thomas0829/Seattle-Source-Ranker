...
echo "[START] Starting Celery workers with multi-token rotation..."
...
if [ -f ~/anaconda3/etc/profile.d/conda.sh ]; then
    ...
    conda activate ssr 2>/dev/null || echo "[WARNING]  Using system Python"
fi

# Load tokens from .env.tokens file
if [ -f .env.tokens ]; then
    echo "[OK] Loading tokens from .env.tokens..."
    export $(grep -v '^#' .env.tokens | xargs)
    
    # Count tokens
    TOKEN_COUNT=$(grep -c "^GITHUB_TOKEN_[0-9]=" .env.tokens)
    echo "[OK] Loaded $TOKEN_COUNT GitHub tokens"
else
    # Fallback to single GITHUB_TOKEN
    if [ -z "$GITHUB_TOKEN" ]; then
        echo "[WARNING]  No .env.tokens file found and GITHUB_TOKEN not set!"
        echo "   Please configure .env.tokens or set GITHUB_TOKEN"
        exit 1
    fi
    echo "[WARNING]  Using single GITHUB_TOKEN (for better performance, configure .env.tokens)"
    export GITHUB_TOKEN
fi
...
nohup python3 -m celery -A seattle_source_ranker.collector.collection_worker worker \
...
echo ""
echo "[STATS] Workers started successfully!"
